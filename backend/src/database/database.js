...
class DatabaseService {
  ...
  async getCountriesWithData() {
    console.log('getCountriesWithData called');
    try {
      console.log('Attempting SQL query');
      // Let's try a very simple query first to see if the table exists
      const testResult = await this.client.execute('SELECT COUNT(*) as count FROM campaigns');
      console.log('Campaigns table count:', testResult.rows[0].count);
      
      // Now try to get distinct countries
      ...
      try {
        ...
        const countries = [...new Set(allCampaigns.filter(c => c.country).map(c => c.country))];
        console.log('Countries from fallback:', countries);
        return countries;
      } catch (fallbackError) {
