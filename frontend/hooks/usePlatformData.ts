...
export const usePlatformData = (platform: string, enabled: boolean = true, country?: string) => {
  ...
  const { data, error, isLoading, mutate } = useSWR<PlatformData | null>(
    ...
    fetcher,
    {
      ...
      errorRetryCount: 1,
      dedupingInterval: 60000, // Increase cache time to 60 seconds to reduce requests
      focusThrottleInterval: 10000, // Increase focus throttle interval
      loadingTimeout: 5000, // Increase loading timeout
      errorRetryInterval: 10000, // Increase error retry interval
